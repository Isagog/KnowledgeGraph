...
public class LinguisticServiceApi  {
   private final LinguisticServiceApiService delegate = LinguisticServiceApiServiceFactory.getLinguisticServiceApi();
    ...
    @io.swagger.annotations.ApiOperation(value = "Sentence extraction", notes = "Extract and analyze sentences from a text", response = SentenceAnnotation.class, responseContainer = "List", tags={ "LinguisticService", })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Success", response = SentenceAnnotation.class, responseContainer = "List"),
        
        @io.swagger.annotations.ApiResponse(code = 401, message = "Unknown graph", response = SentenceAnnotation.class, responseContainer = "List"),
        
        @io.swagger.annotations.ApiResponse(code = 402, message = "Invalid document", response = SentenceAnnotation.class, responseContainer = "List"),
        
        @io.swagger.annotations.ApiResponse(code = 403, message = "Unsupported task", response = SentenceAnnotation.class, responseContainer = "List"),
        
        @io.swagger.annotations.ApiResponse(code = 501, message = "Service unavailable", response = SentenceAnnotation.class, responseContainer = "List"),
        
        @io.swagger.annotations.ApiResponse(code = 502, message = "Server error", response = SentenceAnnotation.class, responseContainer = "List"),
        
        @io.swagger.annotations.ApiResponse(code = 503, message = "Missing implementation", response = SentenceAnnotation.class, responseContainer = "List") })
    public Response documentAnalysis(@ApiParam(value = "", allowableValues="SUMMARY, PASSAGES, FULL", defaultValue="FULL") @DefaultValue("FULL") @QueryParam("mode") ExtractionMode mode
,@ApiParam(value = "", defaultValue="new ArrayList<AnalysisTask>()") @DefaultValue("new ArrayList<AnalysisTask>()") @QueryParam("tasks") List<AnalysisTask> tasks
,@ApiParam(value = "" ) Document document
)
    throws NotFoundException {
        return delegate.documentAnalysis(mode,tasks,document);
    }
    @POST
    ...
    public Response sentenceAnalysis(@ApiParam(value = "Tasks to be executed, all available if missing", defaultValue="new ArrayList<AnalysisTask>()") @DefaultValue("new ArrayList<AnalysisTask>()") @QueryParam("tasks") List<AnalysisTask> tasks
